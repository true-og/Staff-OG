...
public class PunishManager {
    ...
    public void checkNameToUuid(String name, UUID uuid) {
        ...
        try (Connection connection = DatabaseManager.getInstance().createConnection()) {
            ...
            boolean exists = dsl.fetchExists(dsl.select(STAFFOG_HISTORY.ID).from(STAFFOG_HISTORY)
    ...
    public UUID getUUIDFromName(String name) {
        ...
        try (Connection connection = DatabaseManager.getInstance().createConnection()) {
            ...
            DSLContext dsl = DSL.using(connection);
            String uuidString = dsl.select(STAFFOG_HISTORY.UUID).from(STAFFOG_HISTORY)
    ...
    public void newPunishment(PunishEntry entry) {
        ...
        try (Connection connection = DatabaseManager.getInstance().createConnection()) {
                ...
                switch (entry.getType()) {
                    ...
                    case MUTE -> {
                        ...
                        entry.getPlayer().sendMessage(
                                Message.format("&cYour mute " + (entry.getUntil() == -1 ? "does not expire."
                                        : "will expire in: \n" + "&f" + duration)));
            ...
            Message.staffBroadcast(Message.formatNotification(entry.getType().toString(),
                    "New " + entry.getType().toString().toLowerCase() + " on " + entry.getName() + " for " + duration
                            + " by " + entry.getByName()));
    ...
    public PunishEntry getPunishment(PunishType type, int id) {
        ...
        try (Connection connection = DatabaseManager.getInstance().createConnection()) {
            ...
            if (type == PunishType.BAN) {
                ...
                StaffogBanRecord record = dsl.select(STAFFOG_BAN.asterisk()).from(STAFFOG_BAN)
                ...
                StaffogMuteRecord record = dsl.select(STAFFOG_MUTE.asterisk()).from(STAFFOG_MUTE)
    ...
    public void removePunishment(PunishEntry entry) {
        ...
        try (Connection connection = DatabaseManager.getInstance().createConnection()) {
            ...
            Message.staffBroadcast(Message.formatNotification("UN" + entry.getType(), entry.getName() + " has been un"
                    + entry.getType().getBroadcastMessage() + " by " + entry.getRemovedName()));
    ...
    public String createReportFromCR(UUID by, UUID reported, int crid, String reason) {
        ...
        try (Connection connection = DatabaseManager.getInstance().createConnection()) {
            ...
            DSLContext dsl = DSL.using(connection);
            StaffogReportRecord record = dsl.insertInto(STAFFOG_REPORT).set(STAFFOG_REPORT.UUID, reported.toString())
                    .set(STAFFOG_REPORT.BY_UUID, by.toString()).set(STAFFOG_REPORT.TIME, System.currentTimeMillis())
                    .set(STAFFOG_REPORT.TYPE, "CHAT").set(STAFFOG_REPORT.REASON, reason).set(STAFFOG_REPORT.CRID, crid)
                    .returning(STAFFOG_REPORT.ID).fetchOne();
