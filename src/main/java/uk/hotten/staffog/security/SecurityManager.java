...
public class SecurityManager {
    ...
    public StaffIPInfo isIpRecognised(UUID uuid, String ip) {
        ...
        try (Connection connection = DatabaseManager.getInstance().createConnection()) {
            ...
            StaffogStaffipRecord record = dsl.select(STAFFOG_STAFFIP.asterisk()).from(STAFFOG_STAFFIP)
                    .where(STAFFOG_STAFFIP.UUID.eq(uuid.toString())).and(STAFFOG_STAFFIP.IP.eq(ip))
            ...
            return new StaffIPInfo(record.getId(), UUID.fromString(record.getUuid()), record.getIp(),
                    record.getInitial(), record.getPanelAcknowledged(), record.getPanelVerified(),
    ...
    public boolean isUserAccountCreated(UUID id, String ip) {
        ...
        try (Connection connection = DatabaseManager.getInstance().createConnection()) {
            ...
            DSLContext dsl = DSL.using(connection);
            StaffogStaffipRecord record = dsl.select(STAFFOG_STAFFIP.asterisk()).from(STAFFOG_STAFFIP)
                    .where(STAFFOG_STAFFIP.UUID.eq(id.toString())).and(STAFFOG_STAFFIP.IP.eq(ip))
                    .fetchOneInto(STAFFOG_STAFFIP);
    ...
    public boolean hasUUIDGotIp(UUID uuid) {
        ...
        try (Connection connection = DatabaseManager.getInstance().createConnection()) {
            ...
            DSLContext dsl = DSL.using(connection);
            return dsl.fetchExists(dsl.selectFrom(STAFFOG_STAFFIP).where(STAFFOG_STAFFIP.UUID.eq(uuid.toString())));
    ...
    public void createIpEntry(StaffIPInfo info) {
        ...
        try (Connection connection = DatabaseManager.getInstance().createConnection()) {
            ...
            DSLContext dsl = DSL.using(connection);
            dsl.insertInto(STAFFOG_STAFFIP).set(STAFFOG_STAFFIP.IP, info.getIp())
                    .set(STAFFOG_STAFFIP.UUID, info.getUuid().toString()).set(STAFFOG_STAFFIP.INITIAL, info.isInitial())
                    .set(STAFFOG_STAFFIP.PANEL_ACKNOWLEDGED, info.isPanelAcknowledged())
                    .set(STAFFOG_STAFFIP.PANEL_VERIFIED, info.isPanelVerified())
                    .set(STAFFOG_STAFFIP.GAME_VERIFIED, info.isGameVerified()).execute();
    ...
    public void updateIpEntry(StaffIPInfo info) {
        ...
        try (Connection connection = DatabaseManager.getInstance().createConnection()) {
            ...
            DSLContext dsl = DSL.using(connection);
            dsl.update(STAFFOG_STAFFIP).set(STAFFOG_STAFFIP.IP, info.getIp())
                    .set(STAFFOG_STAFFIP.UUID, info.getUuid().toString()).set(STAFFOG_STAFFIP.INITIAL, info.isInitial())
                    .set(STAFFOG_STAFFIP.PANEL_ACKNOWLEDGED, info.isPanelAcknowledged())
                    .set(STAFFOG_STAFFIP.PANEL_VERIFIED, info.isPanelVerified())
                    .set(STAFFOG_STAFFIP.GAME_VERIFIED, info.isGameVerified())
                    .where(STAFFOG_STAFFIP.ID.eq(info.getId())).execute();
    ...
    private boolean doesLinkCodeExist(String code) {
        ...
        try (Connection connection = DatabaseManager.getInstance().createConnection()) {
            ...
            DSLContext dsl = DSL.using(connection);
            return dsl.fetchExists(dsl.selectFrom(STAFFOG_LINKCODE).where(STAFFOG_LINKCODE.CODE.eq(code)));
    ...
    public String doesPlayerHaveLinkCode(UUID uuid) {
        ...
        try (Connection connection = DatabaseManager.getInstance().createConnection()) {
            ...
            DSLContext dsl = DSL.using(connection);
            StaffogLinkcodeRecord record = dsl.select(STAFFOG_LINKCODE.asterisk()).from(STAFFOG_LINKCODE)
                    .where(STAFFOG_LINKCODE.UUID.eq(uuid.toString())).fetchOneInto(STAFFOG_LINKCODE);
    ...
    public String createLinkCode(UUID uuid, boolean admin) {
        ...
        String code = "";
        String possibleChars = "ABCDEFGHIJKLMNOPQRSTUVWXYZ" + "0123456789" + "abcdefghijklmnopqrstuvxyz";
        ...
        try (Connection connection = DatabaseManager.getInstance().createConnection()) {
            ...
            DSLContext dsl = DSL.using(connection);
            dsl.insertInto(STAFFOG_LINKCODE).set(STAFFOG_LINKCODE.UUID, uuid.toString())
                    .set(STAFFOG_LINKCODE.CODE, code).set(STAFFOG_LINKCODE.ADMIN, admin).execute();
    ...
    public void checkAndDeactivateStaffAccount(UUID uuid) {
        ...
        try (Connection connection = DatabaseManager.getInstance().createConnection()) {
            ...
            StaffogWebRecord record = dsl.select(STAFFOG_WEB.asterisk()).from(STAFFOG_WEB)
                    .where(STAFFOG_WEB.UUID.eq(uuid.toString())).fetchOneInto(STAFFOG_WEB);

            if (record == null)
                return;

            dsl.update(STAFFOG_WEB).set(STAFFOG_WEB.ACTIVE, false).where(STAFFOG_WEB.UUID.eq(uuid.toString()))
                    .execute();
