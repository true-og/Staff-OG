...
public class LinkPanelCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        String code = sm.createLinkCode(
                player.getUniqueId(), StaffOGPlugin.getVaultPerms().has(player, "staffog.paneladmin"));
        if (code == null) {

            Message.staffOGMessage(
                    player, ("&cERROR: Failed to create activation code. Please contact an administrator."));
            return true;
        }
